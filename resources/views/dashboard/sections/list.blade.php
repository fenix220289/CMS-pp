<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <a href="{{ route('sections.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Create Section</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200 flex flex-col gap-4">
                    <div class="grid grid-cols-5 gap-4 px-4 py-2 bg-gray-100 rounded-md shadow-md uppercase text-xs">
                    ...
                    <div class="grid grid-cols-5 gap-4 px-4 text-xs">
                        @foreach ($sections as $section)
                            <div class="flex justify-start">
                                <div class="flex flex-row gap-2">
                                    <img src="{{ asset('images/' . $section->image) }}" alt="" class="w-12 h-12 rounded">
                                    <div class="flex flex-col">
                                        <h4 class="text-left">{{ $section->title }}</h4>
                                    </div>
                                </div>
                            </div>
                            <div class="text-center">{!! $section->description !!}</div>
                            <div class="text-center">{!! $section->content !!}</div>
                            <div class="text-center">
                                @if($section->created_at)
                                <x-carbon :date="$section->created_at" human />
                                @else
                                    -
                                @endif
                            </div>
                            <x-dropdown class="text-center text-gray-500 relative flex justify-center">
                                ...
                                <div class="absolute top-10 right-10 z-10 w-20 rounded shadow-xl bg-white px-2 py-1 flex flex-col gap-2">
                                    <a class="hover:bg-gray-100 py-1 px-2 rounded bg-white text-blue-gray-600 flex flex-row gap-2 justify-start items-center" href="{{ route('sections.edit', $section->id) }}">
                                        <span><x-heroicon-o-pencil class="h-4 w-4 text-blue-gray-600" /></span><span>Editar</span>
                                    </a>
                                    <x-form-button :action="route('sections.delete', $section->id)" method="DELETE" class="hover:bg-gray-100 py-1 px-2 rounded bg-white text-red-600 flex flex-row gap-2 justify-start items-center">
